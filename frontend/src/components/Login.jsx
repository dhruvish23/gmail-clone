...
const Login = () => {
    ...
    const submitHandler = async (e) => {
        ...
        try {
            ...
            if (res.data.success) {
                console.log('Local login successful, user data:', res.data.user);
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                console.log('Google login successful, user data from backend:', res.data.user);
    ...
    return (
        <div className='flex item-center justify-center w-screen pt-20'>
            <form onSubmit={submitHandler} className='flex flex-col gap-3 p-4 bg-white rounded-md shadow-md shadow-slate-600 w-[25%]'>
                ...
                <button type='submit' disabled={isLoading} className='bg-gray-800 text-white px-2 py-1 rounded-md'>Login</button>
